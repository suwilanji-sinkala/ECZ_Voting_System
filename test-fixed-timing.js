...
// Test script with fixed timing issue
async function testFixedTiming() {
  console.log('🏛️ Testing Fixed Timing - Complete Election System');
  console.log('==================================================');
  ...
  try {
    ...
    const electionId = 'ELECTION_FIXED_001';
    const currentTime = Math.floor(Date.now() / 1000);
    const startDate = currentTime; // Start now
    const endDate = currentTime + 86400; // Ends in 24 hours
    
    console.log(`Current time: ${currentTime}`);
    console.log(`Start date: ${startDate} (now)`);
    console.log(`End date: ${endDate} (24 hours from now)`);
    
    await contract.methods.createElection(
      electionId,
      'Fixed Timing Election 2024',
      'Election with fixed timing for immediate voting',
      startDate,
      endDate,
      'draft',
      2024,
    ...
    const candidateId = 'CANDIDATE_FIXED_001';
    ...
    const voterId2 = 'VOTER_FIXED_002';
    ...
    console.log('\n🚀 TIMING ISSUE FIXED! Your Ethereum/Ganache setup is now fully functional!');
    console.log('\n📋 Next Steps:');
